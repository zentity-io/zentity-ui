...
export class ModelsTable extends React.Component {
  ...
  onSubmitClone(model, clonedName) {
    ...
    client.get('/_zentity/models/' + model._id)
      .then((response) => {
        try {
          ...
          if (response.data.found === true) {
            ...
            client.post('/_zentity/models/' + clonedName, { data: clonedModel })
              .then((response2) => {
                try {
  ...
  onSubmitCreate(name) {
    ...
    client.put('/_zentity/models/' + name, { data: model })
      .then((response) => {
      ...
      .then((response) => {
        try {
          ...
          if (response.data.result === 'deleted') {
            ...
            const models = [];
            for (var m in this.state.models) {
              if (this.state.models[m]._id === model._id)
                continue;
              models.push(this.state.models[m]);
            }
            this.setState({
  ...
  render() {
    ...
    if (this.state.modalCreate) {
      modalCreate = (
        <EuiConfirmModal
          title={<EuiTitle><h2>Create Model</h2></EuiTitle>}
    ...
    if (this.state.modalClone) {
      modalClone = (
        <EuiConfirmModal
          title={<EuiTitle><h2>Clone Model</h2></EuiTitle>}
        ...
        <EuiFlexGroup gutterSize="s" responsive={false}>
          <EuiFlexItem grow={false}>
            <EuiButton
              ...
              onClick={this.onClickActionCreate}
