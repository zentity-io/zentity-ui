...
const errorToast = (error) => {
  return {
    ...
    text: <p>{error.response ? error.response.data.message : 'Unknown error'}</p>,
  }
}
...
const esQueryToResolutionInput = (esQuery, zentityModelAttributes) => {
  ...
  for (var attrName in esQuery.ast._indexedClauses.field) {
    if (attrValueSets[attrName] === undefined) attrValueSets[attrName] = {}
      ...
      if (clause.match !== 'must' || clause.operator !== 'eq' || clause.type !== 'field')
        throw {
          data: {
            ...
            message:
              'The attribute value query syntax must be the name and value of the attribute separated by a colon.',
          },
        }
  ...
  for (var a in attrs) {
    ...
    for (var v in values) resolutionInput.attributes[attrName].values.push(values[v])
  }
  ...
  for (var t in terms) resolutionInput.terms.push(terms[t])
