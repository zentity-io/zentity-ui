...
export function ModelValidations(props) {
  ...
  for (var i in props.validations) {
    if (props.validations[i].check(props.section, props.state, props.name)) {
      ...
      if (items.length > 0) items.push(<EuiSpacer size="s" key={i + '-spacer'} />)
      items.push(
        <EuiCallOut title={title} color={color} iconType="help" key={i}>
          <p>{props.validations[i].text}</p>
        </EuiCallOut>
      )
    }
  }
  if (items.length === 0) {
    ...
    if (props.validMessage) {
      if (props.validMessage.title) validMessage.title = props.validMessage.title
      if (props.validMessage.text) validMessage.text = props.validMessage.text
    }
    items.push(
